package gr.cite.intelcomp.interactivemodeltrainer.web.scope.user;
...
import gr.cite.intelcomp.interactivemodeltrainer.locale.LocaleService;
...
public class UserInterceptor implements WebRequestInterceptor {
    ...
    public void preHandle(@NotNull WebRequest request) {
        UUID userId = null;
        if (this.currentPrincipalResolver.currentPrincipal() != null && this.currentPrincipalResolver.currentPrincipal().isAuthenticated()) {
            String subjectId = this.claimExtractor.subjectString(this.currentPrincipalResolver.currentPrincipal());

            UserInterceptorCacheService.UserInterceptorCacheValue cacheValue = this.userInterceptorCacheService
                    .lookup(this.userInterceptorCacheService.buildKey(subjectId));
            if (cacheValue != null) {
                userId = cacheValue.getUserId();
            } else {
                userId = this.getUserIdFromDatabase(subjectId);
                if (userId == null)
                    userId = this.createUser(subjectId);
    ...
    private UUID getUserIdFromDatabase(String subjectId) {
